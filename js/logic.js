...
function setup(txt, mName) {
    ...
    i.onkeydown = (e) => {
        ...
        imeS.altBypass = e.altKey;
        if (e.altKey) return;
        if (mod !== 'e' && ((e.code.startsWith('Numpad') && /[0-9]/.test(e.key)) || (/[0-9]/.test(e.key) && !e.ctrlKey))) {
            imeS.nI = true;
            const c = get('ia'), s = c.selectionStart, v = c.value, k = e.key, cl = e.getModifierState('CapsLock');
            const num = cl ? RM[k] : LM[k];
            e.preventDefault();
    ...
    i.onbeforeinput = (v) => {
        ...
        if (mod === 'g' || mod === 'c') {
            ...
            if (k === 'f') {
                ...
                if (val.slice(st - 2, st) === '\u25cc\u093f') {
                    val = val.slice(0, st - 2);
                    st -= 2;
                    char = char + '\u093f';
                    imeS.wI = false
                }
